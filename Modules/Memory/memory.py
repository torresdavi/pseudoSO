...
class Memory:
    ...
    def __validate_process_length(self, process):
        if (self.__validate_process_kernel(process.priority) and process.process_length < 64):
            return True
        elif (not self.__validate_process_kernel(process.priority) and 64 < process.process_length <= 1024):
            return True
        else:
            return False
    ...
    def alocate_memory(self, process):
        ...
        if (process.instruction == 1):
            process.PID = self.id
            self.id += 1
        if (self.__validate_process_kernel(process.priority) and self.__validate_process_length(process)):
            # Verifica se ha processo alocado na memoria (kernel)
            if ((self.memory_block[0] != None) and ((self.offset_kernel + process.process_length) > 64)):
                print("Nao ha espaço para alocacao na memoria para o processo.")
            else:
                if (process.process_length >= 64):
                    print(
                        "Nao foi possível alocar o processo em memória - kernel. Processo muito grande.")
                    return (0)
                else:
                    for i in range(self.offset_kernel, (self.offset_kernel + process.process_length)):
                        self.memory_block[i] = process.PID
            # Passando o offset pra sabermos a localização do processo na memória
            process.own_offset = self.offset_kernel
            self.offset_kernel += process.process_length  # Atualizando o offset do núcleo
        else:
            if (process.process_length >= 960):
                print(
                    "Nao foi possível alocar o processo em memória - user. Processo muito grande.")
                return (0)
            else:
